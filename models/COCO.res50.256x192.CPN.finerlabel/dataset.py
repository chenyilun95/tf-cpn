...
def joints_heatmap_gen_ori(data, label, tar_size=cfg.output_shape, ori_size=cfg.data_shape, points=cfg.nr_skeleton,
...
def joints_heatmap_gen(data, label, tar_size=cfg.output_shape, ori_size=cfg.data_shape, points=cfg.nr_skeleton,
    ...
    for i in range(len(ret)):
        for j in range(points):
            ...
            label[i][j << 1 | 1] = min(label[i][j << 1 | 1], ori_size[0] - 1)
            label[i][j << 1] = min(label[i][j << 1], ori_size[1] - 1)
            ret_tmp[i][j][int( label[i][j << 1 | 1].round()+1e-10) ][ int(label[i][j << 1].round()+1e-10) ] = 1
    for i in range(len(ret)):
        for j in range(points):
            original_label = cv2.GaussianBlur(ret_tmp[i, j], (gaussian_kernel[0]*(ori_size[0]//tar_size[0])+1, 
                gaussian_kernel[1]*(ori_size[0]//tar_size[0])+1), 0)
            ret[i, j] = cv2.resize(original_label, (tar_size[1], tar_size[0]))
    for i in range(len(ret)):
        for j in range(cfg.nr_skeleton):
            am = np.amax(ret[i][j])
            if am <= 1e-8:
