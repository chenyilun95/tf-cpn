...
class ModelDesc(object):
    ...
    @abc.abstractmethod
    def make_inputs(self, is_train):
        pass

    @abc.abstractmethod
    def make_network(self, is_train, inputs):
        pass
...
class Trainer(Base):
    def __init__(self, net, cfg, data_iter):
        self.lr_eval = cfg.lr
    ...
    def enqueue(self, id):
        while True:
            sess.run(self._enqueue_op[id], feed_dict=self.next_feed())

    def queue_inputs(self, make_inputs, id):
        with tf.device('/device:CPU:0'):
            inputs = make_inputs(is_train=True)
            if isinstance(self._inputs, tf.Tensor):
                inputs = [inputs]
            ##### hack
            self.net._inputs = inputs

            self._queue.append( tf.FIFOQueue(capacity=50, dtypes=[i.dtype for i in inputs]) )
            self._enqueue_op.append( self._queue.enqueue(self._inputs) )
            self._dequeue_op.append( self._queue.dequeue() )
            for qi, i in zip(self._dequeue_op[-1], inputs):
                qi.set_shape(i.get_shape())
        return self._dequeue_op[-1]
                    ...
                    with tf.name_scope('tower_%d' % i) as name_scope:
                        ...
                        with slim.arg_scope([slim.model_variable, slim.variable], device='/device:CPU:0'):
                            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane, \
                                ...
                                inputs = self.queue_inputs(self.net.make_inputs, i)
                                self.net.make_network(is_train=True, inputs=inputs)
    ...
    def train(self):
        ...
        self.sess.run(self._queue.close(cancel_pending_enqueues=True))
        coord.request_stop()
        coord.join(threads)
        self.sess.close()
